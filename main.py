...
class HKS(QMainWindow, Ui_MainWindow, find):
    ...
    filename = config["SaveLocaltion"]["savepath"]
    gamepath = config["Location"]["gamepath"]
    save_dir = os.environ["LOCALAPPDATA"] + f"\\..\\LocalLow\\Team Cherry\\Hollow Knight\\{filename}"
    ...
    def read_save(self):
        ...
        if os.path.exists(self.save_dir):
            shutil.copyfile(self.save_dir, f"./bkp_{self.filename}")
            self.textEdit_log.append(f"读取存档:已生成备份存档文件到当前目录{os.getcwd()}\\bkp_{self.filename}")
    ...
    def saveas_file(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "另存为", f"{self.filename}", "存档文件 (*.dat);", options=options)
        if fileName:
    ...
    def opendatajson(self):
        subprocess.run("notepad user_data.json")
        return

    def encoder_save(self):
        save_dir = os.environ["LOCALAPPDATA"] + f"\\..\\LocalLow\\Team Cherry\\Hollow Knight\\{self.filename}"
        if os.path.exists("user_data.json") and os.path.exists(save_dir):
            ...
            if result == QMessageBox.Yes:
                shutil.copyfile(f"./{self.filename}", save_dir)
    ...
    def main(self):
        ...
        self.lab_gamepath.setText(self.gamepath)
        self.lab_gamepath.setWordWrap(True)
        self.lab_savepath.setText(self.save_dir)
        self.lab_savepath.setWordWrap(True)
